...
pub struct PaintableSink {
    ...
    surface: Mutex<Option<Fragile<gdk::Surface>>>,
    ...
    fn propose_allocation(
        ...
        if let Some(f) = caps.features(0) {
            if !f.contains("memory:GLMemory") {
                gst_debug!(
                    CAT,
                    "Failed to get 'memory:GLMemory' feature in caps: {}",
                    caps
                )
            }
        }
        ...
        {
            let gst_context_guard = self.gst_context.lock().unwrap();
            let gst_context = gst_context_guard.as_ref().unwrap();

            // FIXME: real bindings
            let buffer_pool: gst::BufferPool = unsafe {
                let pool = gst_gl::ffi::gst_gl_buffer_pool_new(gst_context.to_glib_none().0);
                assert!(!pool.is_null());
                // Note: its transfer none for gst <= 1.12
    ...
    fn query(&self, element: &Self::Type, query: &mut gst::QueryRef) -> bool {
        ...
        match query.view_mut() {
            gst::QueryView::Context(ref mut q) => {
                let gst_display_guard = self.gst_display.lock().unwrap();
                if let Some(ref display) = *gst_display_guard {
                    ...
                    // FIXME: real bindings
                    unsafe {
                        let res = gst_gl::ffi::gst_gl_handle_context_query(
...
impl PaintableSink {
    pub(crate) fn realize_context(&self, obj: &super::PaintableSink) {
        ...
        let mut gdk_context_guard = self.gdk_context.lock().unwrap();
        gdk_context_guard.replace(ctx);
    }
