...
impl PaintableSink {
    ...
    fn initialize_gl(obj: glib::WeakRef<Self>) -> Result<(), glib::Error> {
        ...
        let ctx = ctx_guard
            .as_ref()
            .expect("Trying to initialize GL withotu GDK Context")
            .get();
        ...
        if gl_ctx != 0 {
            ...
            unsafe {
                ...
                let wayland_display =
                    gdk_wayland_sys::gdk_wayland_display_get_wl_display(d.to_glib_none().0);
                assert!(!wayland_display.is_null());

                let gst_display =
                    gst_gl_wayland_sys::gst_gl_display_wayland_new_with_display(wayland_display);
                assert!(!gst_display.is_null());
                let gst_display: gst_gl::GLDisplay =
                    from_glib_full(gst_display as *mut gst_gl_sys::GstGLDisplay);
